...
async function main() {
  ...
  const k1 = await prisma.league.upsert({
    where: { slug: "kleague1" },
    update: {},
    create: { name: "K League 1", slug: "kleague1" },
  })
  const k2 = await prisma.league.upsert({
    where: { slug: "kleague2" },
    update: {},
    create: { name: "K League 2", slug: "kleague2" },
  })
  console.log("Leagues:", k1.slug, k2.slug)

  // -------------------------------------------------------------------------
  // Teams (TEAM_LIST.md 순서·이름)
  // -------------------------------------------------------------------------
  const slug = (league: string, emblem: string) => `${league}-${emblem}`
  const teamK1: Record<string, { id: string }> = {}
  for (const t of K1_TEAMS) {
    const team = await prisma.team.upsert({
      where: { slug: slug("kleague1", t.emblem) },
      update: { name: t.name, emblemPath: EMBLEM("kleague1", t.emblem) },
      create: {
        name: t.name,
        slug: slug("kleague1", t.emblem),
  ...
  for (const t of K2_TEAMS) {
    const team = await prisma.team.upsert({
      where: { slug: slug("kleague2", t.emblem) },
      update: { name: t.name, emblemPath: EMBLEM("kleague2", t.emblem) },
      create: {
        name: t.name,
        slug: slug("kleague2", t.emblem),
  ...
  // -------------------------------------------------------------------------
  // Rounds (1라운드: MATCH_SCHEDULE.md, 5라운드: 샘플용)
  // -------------------------------------------------------------------------
  const now = new Date()
  ...
  const round1Start = new Date("2026-02-28T00:00:00Z")
  const round1End = new Date("2026-03-02T23:59:59Z")

  const k1Round1 = await prisma.round.upsert({
    where: { leagueId_number: { leagueId: k1.id, number: 1 } },
    update: { startAt: round1Start, endAt: round1End },
    create: {
      leagueId: k1.id,
      number: 1,
      startAt: round1Start,
  ...
  const k2Round1 = await prisma.round.upsert({
    where: { leagueId_number: { leagueId: k2.id, number: 1 } },
    update: { startAt: round1Start, endAt: round1End },
    create: {
      leagueId: k2.id,
      number: 1,
  ...
  const r1 = await prisma.round.upsert({
    where: { leagueId_number: { leagueId: k1.id, number: 5 } },
    update: { startAt: round5Start, endAt: round5End, isCurrent: true },
    create: {
      leagueId: k1.id,
      number: 5,
      startAt: round5Start,
  ...
  await prisma.round.upsert({
    where: { leagueId_number: { leagueId: k2.id, number: 5 } },
    update: { startAt: round5Start, endAt: round5End, isCurrent: true },
    create: {
      leagueId: k2.id,
      number: 5,
  ...
  console.log("Rounds: K1/K2 R1 (MATCH_SCHEDULE), K1/K2 R5 (isCurrent=true)")
  ...
  if (k1Round1MatchCount === 0) {
    for (const m of K1_ROUND1_MATCHES) {
  ...
  if (k2Round1MatchCount === 0) {
    for (const m of K2_ROUND1_MATCHES) {
  ...
  for (const { name, link } of refereeList) {
    ...
    if (existing) {
      ...
      await prisma.referee.create({ data: { name, link } })
    }
    refCount++
  }
  ...
  if (liveMatch && ref고형진 && ref설태환) {
    ...
    if (!existing) {
      await prisma.matchReferee.create({
        data: { matchId: liveMatch.id, refereeId: ref고형진.id, role: "MAIN_REFEREE" },
      })
      await prisma.matchReferee.create({
        data: { matchId: liveMatch.id, refereeId: ref설태환.id, role: "VAR" },
      })
    }
  }
  if (finishedMatch && ref채상협) {
    ...
    if (!existing) {
      await prisma.matchReferee.create({
        data: { matchId: finishedMatch.id, refereeId: ref채상협.id, role: "MAIN_REFEREE" },
      })
    }
  }
