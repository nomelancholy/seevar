...
export function RoundRefereeBestWorstSection({
  ...
  const [bestWorstOpen, setBestWorstOpen] = useState<Set<string>>(
    () => new Set(ROLE_ORDER.filter((r) => DEFAULT_OPEN_BEST_WORST.has(r))),
  );
  ...
  const renderCard = (data: RefereeCardData, kind: "best" | "worst") => {
          ...
          <div className="min-w-0 flex-1">
            ...
            {data.matchForDisplay && (
              ...
              >
                ...
                {data.matchForDisplay.awayName} →
              </Link>
            )}
          </div>
          <div className="text-right shrink-0">
            <span
              className={`${badgeBg} px-2.5 py-1 text-[10px] md:text-xs font-black uppercase`}
            >
              {isBest ? "최고점" : "최저점"}
            </span>
        ...
        {data.reviews.length > 0 && (
          <div className="space-y-4">
            ...
            {data.reviews.map((fb) => (
              <div
                key={fb.id}
                className="bg-zinc-900/50 p-3 border border-zinc-800"
              >
                <div className="flex justify-between items-center mb-2">
                  ...
                  <div className="flex flex-col items-end gap-1">
                    ...
                    <div className="flex items-center gap-1 text-blue-400">
                      ...
                      <span className="font-mono text-xs md:text-sm font-black">
                        {fb.likeCount}
                      </span>
                    </div>
                  </div>
  ...
  const renderBestWorstRow = (role: string) => {
    ...
    const refCountForRole = allRoundReferees.filter(
      (r) => r.role === role,
    ).length;
            ...
            {(best || worst) && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 p-4 md:p-5">
                {best ? (
                  ...
                  <div className="ledger-surface p-4 border border-border text-muted-foreground font-mono text-sm">
                    해당 역할 베스트 없음
                  </div>
                )}
                {worst ? (
            ...
            {refCountForRole > 0 && (
              <div className="border-t border-border/50">
                ...
                >
                  ...
                  <span className="text-muted-foreground/80 font-normal text-[10px]">
                    ({refCountForRole}명)
                  </span>
                </button>
                {fullRatingOpen === role && (
        ...
        <table className="w-full min-w-0 md:min-w-[520px] font-mono text-xs md:text-sm">
          <thead>
            <tr className="text-left text-muted-foreground border-b border-border">
              ...
              <th className="py-2 pr-2 md:pr-3 font-bold uppercase tracking-wider">
                경기
              </th>
              <th className="py-2 pr-2 md:pr-3 font-bold uppercase tracking-wider whitespace-nowrap">
                역할
            ...
            {filtered.map((ref) => (
              ...
              >
                  ...
                  {ref.matchForDisplay ? (
                    ...
                    >
                      ...
                      <span className="hidden md:inline">
                        {ref.matchForDisplay.homeName} vs{" "}
                        {ref.matchForDisplay.awayName}
                      </span>
                    </Link>
                  ) : (
                ...
                <td className="hidden md:table-cell py-2.5 pr-3 text-right tabular-nums text-muted-foreground align-middle">
                  {ref.homeAvg != null
                    ? `${ref.homeAvg.toFixed(1)} / 5.0`
                    : "—"}
                </td>
                <td className="hidden md:table-cell py-2.5 pr-3 text-right tabular-nums text-muted-foreground align-middle">
                  {ref.awayAvg != null
                    ? `${ref.awayAvg.toFixed(1)} / 5.0`
                    : "—"}
                </td>
  ...
  const hasAnyData = ROLE_ORDER.some(
    (r) =>
      bestByRole[r] ||
      worstByRole[r] ||
      allRoundReferees.some((ref) => ref.role === r),
  );
